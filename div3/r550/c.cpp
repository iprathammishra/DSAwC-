...
int main() {
  ...
  for (int i = 1; i < n; i++) {
    if (a[i] > inc.back()) { inc.push_back(a[i]); s.insert(i); }
  }
  vector<int> dec;
  for (int i = n-1; i >= 0; i--) {
    if (s.count(i)) { continue; }
    if (dec.empty()) { dec.push_back(a[i]); }
    else if (a[i] < dec.back()) { dec.push_back(a[i]); }
  }
  if ((int) inc.size() + (int) dec.size() != n) { cout << "NO" << endl; }
  else {
    cout << "YES" << endl;
    cout << (int) inc.size() << endl;
    for (int& i : inc) { cout << i << " "; }
    cout << endl << (int) dec.size() << endl;
